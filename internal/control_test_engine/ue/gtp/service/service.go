...
func SetupGtpInterface(ue *context.UEContext, msg gnbContext.UEMessage) {
	...
	if pduSession == nil || err != nil {
		log.Error("[GNB][GTP][]()()(", ue.GetPrUeId(), ") Aborting the setup of PDU Session ", gnbPduSession.GetPduSessionId(), ", this PDU session was not succesfully configured on the UE's side.")
		return
	}
	pduSession.GnbPduSession = gnbPduSession

	if ue.TunnelMode == config.TunnelDisabled {
		log.Info(fmt.Sprintf("[UE][GTP][]()()(", ue.GetMsin(), ")Interface for UE %s has not been created. Tunnel has been disabled.", ue.GetMsin()))
		return
	}

	if pduSession.Id != 1 {
		log.Warn("[GNB][GTP][]()()(", ue.GetPrUeId(), ") Only one tunnel per UE is supported for now, no tunnel will be created for second PDU Session of given UE")
		return
	}
	...
	go func() {
		...
		if err := gtpLink.CmdAdd(nameInf, 1, ueGnbIp.String(), stopSignal); err != nil {
			log.Fatal("[GNB][GTP][]()()(", ue.GetPrUeId(), ") Unable to create Kernel GTP interface: ", err, msin, nameInf)
			return
		}
	}()
	...
	log.Debug("[UE][GTP][]()()(", ue.GetMsin(), ")Setting up GTP Forwarding Action Rule for ", strings.Join(cmdAddFar, " "))
	if err := gtpTunnel.CmdAddFAR(cmdAddFar); err != nil {
		log.Fatal("[GNB][GTP][]()()(", ue.GetPrUeId(), ") Unable to create FAR: ", err)
		return
	}
	...
	log.Debug("[UE][GTP][]()()(", ue.GetMsin(), ")Setting up GTP Forwarding Action Rule for ", strings.Join(cmdAddFar, " "))
	if err := gtpTunnel.CmdAddFAR(cmdAddFar); err != nil {
		log.Fatal("[UE][GTP][]()()(", ue.GetMsin(), ")Unable to create FAR ", err)
		return
	}
	...
	log.Debug("[UE][GTP][]()()(", ue.GetMsin(), ")Setting up GTP Packet Detection Rule for ", strings.Join(cmdAddPdr, " "))
	...
	if err := netlink.AddrAdd(link, addrTun); err != nil {
		log.Fatal("[UE][DATA][]()()(", ue.GetMsin(), ")Error in adding IP for virtual interface", err)
		return
	}
	...
	case config.TunnelTun:
		...
		if err := netlink.RuleAdd(rule); err != nil {
			log.Fatal("[UE][DATA][]()()(", ue.GetMsin(), ")Unable to create routing policy rule for UE", err)
			return
		}
	...
	case config.TunnelVrf:
		...
		if err := netlink.LinkAdd(vrfDevice); err != nil {
			log.Fatal("[UE][DATA][]()()(", ue.GetMsin(), ")Unable to create VRF for UE", err)
			return
		}

		if err := netlink.LinkSetMaster(link, vrfDevice); err != nil {
			log.Fatal("[UE][DATA][]()()(", ue.GetMsin(), ")Unable to set GTP tunnel as slave of VRF interface", err)
			return
		}

		if err := netlink.LinkSetUp(vrfDevice); err != nil {
			log.Fatal("[UE][DATA][]()()(", ue.GetMsin(), ")Unable to set interface VRF UP", err)
			return
		}
	...
	if err := netlink.RouteReplace(route); err != nil {
		log.Fatal("[GNB][GTP][]()()(", ue.GetPrUeId(), ") Unable to create Kernel Route ", err)
	}
