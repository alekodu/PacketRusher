...
func InitRegistration(ue *context.UEContext) {
	log.Info("[UE][]()()(", ue.GetPrUeId(), ") Initiating Registration")
	...
	if len(ue.UeSecurity.Kamf) != 0 {
		...
		if err != nil {
			log.Fatalf("[UE][NAS]()()(", ue.GetPrUeId(), ") Unable to encode with integrity protection Registration Request: %s", err)
		}
	}
	// send to GNB.
...
func InitPduSessionRequest(ue *context.UEContext) {
	log.Info("[UE][]()()(", ue.GetPrUeId(), ") Initiating New PDU Session")
...
func InitPduSessionRequestInner(ue *context.UEContext, pduSession *context.UEPDUSession) {
	...
	if err != nil {
		log.Fatal("[UE][NAS]()()(", ue.GetPrUeId(), ") Error sending ul nas transport and pdu session establishment request: ", err)
	}
...
func InitPduSessionRelease(ue *context.UEContext, pduSession *context.UEPDUSession) {
	log.Info("[UE][]()()(", ue.GetPrUeId(), ") Initiating Release of PDU Session ", pduSession.Id)

	if pduSession.GetStateSM() != context.SM5G_PDU_SESSION_ACTIVE {
		log.Warn("[UE][NAS]()()(", ue.GetPrUeId(), ") Skipping releasing the PDU Session ID ", pduSession.Id, " as it's not active")
		return
	}
...
func InitPduSessionReleaseComplete(ue *context.UEContext, pduSession *context.UEPDUSession) {
	log.Info("[UE][]()()(", ue.GetPrUeId(), ") Initiating PDU Session Release Complete for PDU Session", pduSession.Id)

	if pduSession.GetStateSM() != context.SM5G_PDU_SESSION_INACTIVE {
		log.Warn("[UE][NAS]()()(", ue.GetPrUeId(), ") Unable to send PDU Session Release Complete for a PDU Session which is not inactive")
		return
	}
...
func InitDeregistration(ue *context.UEContext) {
	log.Info("[UE]()()(", ue.GetPrUeId(), ") Initiating Deregistration")

	// registration procedure started.
	deregistrationRequest, err := mm_5gs.DeregistrationRequest(ue)
	if err != nil {
		log.Fatal("[UE][NAS]()()(", ue.GetPrUeId(), ") Error sending deregistration request: ", err)
	}
...
func InitIdentifyResponse(ue *context.UEContext) {
	log.Info("[UE]()()(", ue.GetPrUeId(), ") Initiating Identify Response")
...
func InitConfigurationUpdateComplete(ue *context.UEContext) {
	log.Info("[UE]()()(", ue.GetPrUeId(), ") Initiating Configuration Update Complete")

	// trigger Configuration Update Complete.
	identityResponse, err := mm_5gs.ConfigurationUpdateComplete(ue)
	if err != nil {
		log.Fatal("[UE][NAS]()()(", ue.GetPrUeId(), ") Error sending Configuration Update Complete: ", err)
	}
	// send to GNB.
	sender.SendToGnb(ue, identityResponse)
}
...
func InitServiceRequest(ue *context.UEContext) {
	log.Info("[UE]()()(", ue.GetPrUeId(), ") Initiating Service Request")
...
func SwitchToIdle(ue *context.UEContext) {
	log.Info("[UE]()()(", ue.GetPrUeId(), ") Switching to 5GMM-IDLE")
